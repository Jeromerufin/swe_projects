...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        for (int i = 0; i < piece.height(); i += 1) {
            for (int j = 0; j < piece.width(); j += 1) {
                if (row + i >= _height || col + j >= _width) {
                    return false;
                } else if (piece.get(i, j) && _cells[row + i][col + j]) {
                    return false;
                }
            }
        }
    ...
    /** Return an array result such that result[0][r] is the number of
     *  filled grid squares in row r and result[1][c] is the number of
    ...
    void clearFilledLines() {
        ...
        for (int row: rowsToClear) {
            _cells[row] = new boolean[_height];
        }

        boolean[][] cellsT = transpose(_cells);

        for (int col: colsToClear) {
            cellsT[col] = new boolean[_height];
        }

        _cells = transpose(cellsT);

        _score += scoreClearedLines(nrows, ncols);
    }

    boolean[][] transpose(boolean[][] m) {
        boolean[][] mT = new boolean[m[0].length][m.length];
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int regClearScore = (nrows * _height) + (ncols * (_height - nrows));
        int bonusScore = _height * (nrows + ncols) * _streakLength;
        return regClearScore + bonusScore;
    }
    ...
    void deal(Piece piece) {
        if (!_hand.isEmpty() && _hand.get(0) == null) {
            _hand = new ArrayList<>();
        }
        _hand.add(piece);
    }
